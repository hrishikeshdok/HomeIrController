...
public class VolumeStepController implements ApplianceController {
    ...
    public void handle(JSONObject directive, ConsumerIrManager irManager) {
        ...
        try {
            ...
            if ("SetVolume".equals(directiveName))
            {
                ...
                Log.d(TAG, "DesiredVolume = " + desiredVolume);
            ...
            {
                int volumeSteps = payloadObject.getInt("volumeSteps");
                ...
                if (volumeDefault)
                {
                    Log.d(TAG, "Changing volume by default value ");
                    for (int i = 0; i<3; i++)
                    {
                        Thread.sleep(10);
                    ...
                    Log.d(TAG, "Changing volume by " + volumeSteps);
    ...
    static VolumeStepController getInstance()
    {
        if (instance == null)
        {
    ...
    public String getName() {
        return VolumeStepController.class.getSimpleName();
    }

    private VolumeStepController()
    {}

    private static VolumeStepController instance;
    private static IrBlaster irBlaster;
    private static final String TAG = "VolumeStepController";
}
